...
describe('settings', () => {
  ...
  test('defaults disable automatic interval renew on first install', async () => {
    const settings = new Settings(new StorageAreaMock('some-key', 'local'))
    const current = await settings.get()

    expect(current.renew.enabled).toBeFalsy()
    expect(current.renew.intervalMillis).toBe(10 * 60 * 1000)
  })
  ...
  test('update does not mutate defaults across resets', async () => {
    ...
    expect(fresh.fingerprint.cssMediaQuery.mode).toBe('real')
    expect(fresh.fingerprint.cssMediaQuery.preset).toBe('1920x1080@1')
    expect(fresh.fingerprint.localeMode).toBe('random')
    expect(fresh.generator.syncOsWithHost).toBe(false)
  })
  ...
  test('partial from storage', async () => {
    ...
    await settings.update({ enabled: false, fingerprint: { cssMediaQuery: { mode: 'static', preset: '390x844@3' } } }) // force to save
    ...
    settings.onChange((s) => {
      expect(s.fingerprint.cssMediaQuery.mode).toEqual('static')
