...
export default class {
  ...
  constructor(storage: StorageArea<SettingsState>, browserName?: 'chrome' | 'firefox' | 'opera' | 'edge' | 'safari') {
    ...
    switch (browserName) {
      ...
      case 'safari':
        this.defaults.generator.types = ['safari_iphone', 'safari_mac']
        this.defaults.generator.syncOsWithHost = true
        break
    }
  }
  ...
  async update(updated: PartialSettingsState): Promise<ReadonlySettingsState> {
    ...
    const [current] = this.merge(structuredClone(this.defaults), (await this.storage.get()) ?? this.defaults)
