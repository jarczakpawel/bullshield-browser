  ...
  for (let i = 1; i <= 30; i++) {
    ...
    describe('chrome_android', () => {
      test('auto', () => {
        ...
        expect(result.device?.osVersion).not.toBe('')
        expect(result.userAgent).toMatch(generalUserAgentRe)
        expect(result.userAgent).contains(`Android ${result.device?.osVersion}`)
        expect(result.userAgent).contains(result.device?.model || '')
        expect(result.userAgent).contains('Linux')
        expect(result.userAgent).contains('AppleWebKit/537.36 (KHTML, like Gecko) Chrome/')
      })

      test('specific', () => {
        const result = generate('chrome_android', { version: '90', mobileVendor: 'Pixel 8' })
        ...
        expect(result.userAgent).contains('Linux')
        expect(result.userAgent).contains('Pixel 8')
        expect(result.userAgent).contains('AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90')
      })

      test('caps chrome on android 8 and 9', () => {
        const result = generate('chrome_android', { mobileVendor: 'Pixel 3', maxMajor: 160 })

        expect(result.device?.model).toBe('Pixel 3')
        expect(result.device?.osVersion).toBe('9')
        expect(result.version.browser.major).toBeLessThanOrEqual(138)
      })
    })
    ...
    describe('firefox_win', () => {
      test('auto', () => {
        ...
        expect(result.userAgent).toMatch(generalUserAgentRe)
        expect(result.userAgent).contains('Windows NT')
        expect(result.userAgent).toMatch(/Windows NT 10\.0; (?:Win64; x64|WOW64)/)
        expect(result.userAgent).contains('Gecko/')
        expect(result.userAgent).contains('Firefox/')
      })
    ...
    describe('firefox_android', () => {
      ...
      test('specific', () => {
        const result = generate('firefox_android', { version: '1.2.3', mobileVendor: 'Pixel 8 Pro' })

        expect(result.version.browser.full).toBe('1.2.3')
        expect(result.version.browser.major).toBe(1)
        expect(result.version.underHood).toBeUndefined()
        expect(result.device?.model).toBe('Pixel 8 Pro')
        expect(result.userAgent).toMatch(generalUserAgentRe)
        expect(result.userAgent).contains('Android')
        expect(result.userAgent).contains('Pixel 8 Pro')
        expect(result.userAgent).contains('Gecko/')
